...
def find_chunk_json_files() -> list[Path]:
    """
    모든 공고의 HWP/HWPX Chunk JSON을 찾습니다.

    Parser/Normalizer/Structure/Chunk 결과가 존재하는 모든 형식을
    각각 임베딩하여 다음 경로에 분리 저장합니다.

    outputs/<document_id>/05_embeddings/hwp/
    outputs/<document_id>/05_embeddings/hwpx/
    """
    ...
    for document_id, data in groups.items():
        ...
        for document_format in ("hwp", "hwpx"):
            ...
            if chunk_path.is_file():
                files.append(chunk_path.resolve())
...
def embed_all() -> None:
    ...
    print()
    print("=" * 70)
    print("전체 HWP/HWPX Chunk Embedding")
    print("=" * 70)
    print(f"대상: {len(files)}개")
...
def print_menu() -> None:
    ...
    print("9. 전체 HWP/HWPX Chunk JSON 임베딩")
