...
export function DetailScreen({ go, showToast, notice }: { go: (s: any) => void; showToast: (m: string) => void; notice?: any; }) {
  ...
  const eligibilityData = [["신청 자격", "무주택 세대구성원 등"], ["소득 기준", "공고문 세부 요건 참조"]];
  const docsData = ["주민등록등본", "가족관계증명서", "소득금액증명원 등"];

  // 💡 초기 대화도 시연용 공고에 맞춰 수정
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: "user", text: "이 공고의 신청 자격을 알려줘.", time: "10:30" },
    { role: "ai", text: `[${currentNotice.title}] 공고를 기준으로 확인한 결과, 무주택 세대구성원 등이 신청 대상에 포함됩니다. 자세한 사항은 질문해주세요.`, time: "10:30", evidence: true }
  ]);
  ...
  const send = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/chat`, {
        ...
        // 💡 실제 AWS 백엔드로 notice_id와 질문 전송
        body: JSON.stringify({ notice_id: currentNotice.id, question: userMsg })
      });
      if (!response.ok) throw new Error();
      const data = await response.json();
      
      // 💡 백엔드에서 받은 실제 답변 적용
      setMessages((m) => [...m, { role: "ai", text: data.answer, time: now, evidence: data.has_evidence }]);
    } catch {
  ...
  return (
    <UserLayout screen="detail" go={go} showToast={showToast}>
              ...
              <div key={i} className="flex items-start gap-2.5 mb-5">
                ...
                <div className="flex flex-col items-start max-w-[80%] lg:max-w-[70%]">
                  <div className="bg-white border border-slate-200 text-slate-800 text-[14px] lg:text-[15px] leading-relaxed px-4 py-3 rounded-2xl rounded-tl-sm shadow-sm whitespace-pre-wrap break-keep">
                    {m.text}
                    {m.evidence && (
                      <button onClick={() => setEvidence(true)} className="block mt-3 bg-blue-50 text-blue-600 text-[13px] font-bold px-3 py-1.5 rounded-md hover:bg-blue-100 transition-colors">
                        근거 문단 보기
                      </button>
                    )}
                  </div>
      ...
      {evidence && (
        <InfoModal title="답변 근거" onClose={() => setEvidence(false)}>
          <p className="text-slate-500 text-sm mb-4">AI 답변에 사용된 원문 문단입니다.</p>
          <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 mb-4 text-[13px]">
            <div className="grid grid-cols-[80px_1fr] gap-2 mb-2"><b className="text-slate-700">근거 유형</b><span className="text-slate-600">신청 자격</span></div>
            <div className="grid grid-cols-[80px_1fr] gap-2"><b className="text-slate-700">위치</b><span className="text-slate-600">공고문 신청자격 섹션</span></div>
          </div>
          <blockquote className="border-l-4 border-blue-500 bg-blue-50/50 p-4 text-[15px] text-slate-800 leading-relaxed mb-6 rounded-r-lg">
            입주자 모집공고일 현재 무주택 세대구성원으로서 소득 및 자산 기준을 만족하는 자를 신청 대상으로 합니다.
          </blockquote>
          <div className="flex justify-end">
            <button onClick={() => setEvidence(false)} className="bg-blue-600 text-white font-bold px-5 py-2.5 rounded-lg hover:bg-blue-700 transition-colors">닫기</button>
          </div>
        </InfoModal>
      )}
    </UserLayout>
